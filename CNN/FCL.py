...
class FCL:
    ...
    def apply(self, input_tensor):
        ...
        z = np.dot(input_vector, self.weights) + self.biases 
    ...
    def apply_batch(self, input_tensor):
        ...
        # print("k")
        # print(self.last_input_shape)

        input_flat = np.zeros((input_tensor.shape[0], self.weights.shape[0]))
        z = np.zeros((input_tensor.shape[0], self.weights.shape[1]))
        ...
        for b in range(input_tensor.shape[0]):
            z[b] = np.dot(input_flat[b], self.weights)
    ...
    #     dLoss_dOutput = self.backprop_znorm_batch(dLoss_dOutput, learn_rate)
    ...
    #     # print(dLoss_dOutput.shape)
    #     # # print(dOutput_dInput.shape)
    #     # # print(dOutput_dBias.shape)
    #     # print(dOutput_dWeights.shape)

    #     dLoss_dWeights   = np.zeros((self.last_input.shape[0], self.weights.shape[0], self.weights.shape[1]))
    #     # dLoss_dInput     = np.zeros((self.last_input.shape[0], self.weights.shape[0], 1))

    #     dLoss_dWeights = dLoss_dOutput.T @ dOutput_dWeights
    #     dLoss_dInput   = dLoss_dOutput @ dOutput_dInput.T

    #     # print(dLoss_dWeights.shape)
    #     # print(dLoss_dBias.shape)

    #     self.weights -= learn_rate * dLoss_dWeights.T

    #     return dLoss_dInput.reshape(self.last_input_shape)

    def backprop_batch(self, dLoss_dOutput, learn_rate):
        dLoss_dOutput = dLoss_dOutput
        dOutput_dInput   = self.weights
        #dOutput_dBias    = np.full(shape=(self.biases.shape[0], 1), fill_value=1)
        #dOutput_dWeights = np.sum(self.last_input[:, np.newaxis], axis=0) / self.last_input_shape[0]
        dOutput_dWeights = self.last_input

        print("//////////////////")
        print(dLoss_dOutput.T.shape)
        print(dOutput_dInput.shape)
        print(dOutput_dWeights.shape)


        #dLoss_dWeights   = np.outer(dLoss_dOutput, dOutput_dWeights.T)
        dLoss_dWeights   = dLoss_dOutput.T @ dOutput_dWeights / self.last_input_shape[0]
        #dLoss_dBias      = dLoss_dOutput
        dLoss_dInput     = dOutput_dInput @ dLoss_dOutput.T
    ...
    def backprop(self, dLoss_dOutput, learn_rate):
        ...
        dLoss_dOutput = dLoss_dOutput[:, np.newaxis]
        dOutput_dInput   = self.weights
        dOutput_dBias    = np.full(shape=(self.biases.shape[0], 1), fill_value=1)
        dOutput_dWeights = self.last_input[:, np.newaxis]
        ...
        dLoss_dWeights   = np.outer(dLoss_dOutput, dOutput_dWeights.T)
        dLoss_dBias      = dLoss_dOutput
        dLoss_dInput     = dOutput_dInput @ dLoss_dOutput
