...
class ModelLoader:
    ...
    def save_model(self, fname):
        ...
        with open(fname + "_conv_filters.csv", mode='w') as filter_file:
            ...
            for f in self.nn.conv_layer.filters:
                print(f)
                to_write = []
        ...
        with open(fname + "_fcl.csv", mode='w') as fcl_file:
            ...
            for i in range(self.nn.fcl_layer.weights.shape[0]):
                to_write = []
                for j in range(self.nn.fcl_layer.weights.shape[1]):
                    to_write.append(self.nn.fcl_layer.weights[i, j])
                fcl_writer.writerow(to_write)
            
            fcl_writer.writerow([b for b in self.nn.fcl_layer.biases])
    ...
    def load_model(self, fname):
        ...
        with open(fname + "_conv_filters.csv", mode='r') as filter_file:
            ...
            for row in filter_reader:
                for idx in range(len(row)):
                    j = idx % 3
                    i = idx // 3
                    self.nn.conv_layer.filters[f][i][j] = float(row[idx])
                f += 1
        ...
        with open(fname + "_fcl.csv", mode='r') as fcl_file:
            ...
            for row in fcl_reader:
                if matrix_row == self.nn.fcl_layer.weights.shape[0]:
                    for idx in range(len(row)):
                        self.nn.fcl_layer.biases[idx] = float(row[idx])
                else:
                    for idx in range(len(row)):
                        self.nn.fcl_layer.weights[matrix_row][idx] = float(row[idx])
                    matrix_row += 1