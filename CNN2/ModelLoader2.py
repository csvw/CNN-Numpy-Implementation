...
class ModelLoaderMR:
    ...
    def save(self, name):
        self.save_fcl(name, self.nn.fcl, 1)
    ...
    def load(self, name):
        self.load_fcl(name, self.nn.fcl, 1)

    def load_fcl(self, name, fcl, num):
        with open(name + "_fcl.csv", mode='r') as fcl_file:
        ...
        with open(name + "_conv_filters" + str(num) + ".csv", mode='r') as filter_file:
            ...
            for row in filter_reader:
                for idx in range(len(row)):
                    j = idx % 3
                    i = idx // 3
                    conv.filters[i][j][f] = float(row[idx])
                f += 1