...
class ShapeData:
    def __init__(self):
        ...
        self.cmp_path = self.cwd / "comparisons2"
        self.rot_pair_path = self.cwd / 'rot_pairs'
        self.cre_pair_path = self.cwd / 'create_pairs'
        self.des_pair_path = self.cwd / 'destroy_pairs'
        self.ref_pair_path = self.cwd / 'ref_pairs'
        self.ide_pair_path = self.cwd / 'idem_pairs'
        self.paths = [self.rot_pair_path, self.cre_pair_path, self.des_pair_path, self.ref_pair_path, self.ide_pair_path]
        self.pair_imgs = list()
        self.rot_imgs = list()
        self.ref_imgs = list()
        self.num_imgs = list()
        self.shp_imgs = list()
        self.cmp_imgs = list()
        # self.get_cmp_imgs(self.cmp_path, self.cmp_imgs)
    ...
    def transformation_name(self, f):
        n = f.name
        if n == 'ref_pairs':
            return "Reflection", 0
        if n == 'rot_pairs':
    ...
    def get_pair_imgs(self):
        for path in self.paths:
            name, c = self.transformation_name(path)
            count = 0
            for f in path.iterdir():
                img_path = path / f.name
                label = [1 if i == c else 0 for i in range(4)]
                img = Image.open(img_path)
                img = img.resize((48, 96))
                self.pair_imgs.append((label, np.array(img)))
                count += 1
                if count == 10000:
                    break
        random.shuffle(self.pair_imgs)
    ...
    def get_cmp_imgs(self, path, imgs):
        idx = 0
        for f in path.iterdir():
            img_path = path / f.name
            imgs.append((self.cmp_label(f.name), np.array(Image.open(img_path))))
    ...
    def init_cmps(self):
        self.cmp_data_batch = []
        for i in range(0, len(self.cmp_imgs), 1):
            imgs   = []
            labels = []
            for img in self.cmp_imgs[i:i+1]:
            ...
            labels = labels[0]
            batch = [labels, tensor]
            self.cmp_data_batch.append(batch)
        random.shuffle(self.cmp_data_batch)
